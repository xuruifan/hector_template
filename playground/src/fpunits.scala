...
class DivFBase(latency: Int, expWidth: Int, sigWidth: Int) extends MultiIOModule {
  ...
  val fakeModule = Module(new MulFBase(latency, expWidth, sigWidth))
  fakeModule.operand0 := operand0
  fakeModule.operand1 := operand1
  fakeModule.ce := ce
  result := fakeModule.result
...
object NegF {
  def apply(expWidth: Int, sigWidth: Int, in: Bits): UInt = {
    val fWidth = expWidth + sigWidth
